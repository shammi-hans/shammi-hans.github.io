...
$allowedExts = array("doc","xlsx", "docx", "odt", "pdf", "zip", "rar");
$temp = explode(".", $_FILES["image_file"]["name"]);
$extension = end($temp);
if ((($_FILES["image_file"]["type"] == "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
...
&& ($_FILES["image_file"]["size"] < 1000000) //20000 == 20kb currently ~1MB
&& in_array($extension, $allowedExts))
  ...
  {
  echo "Invalid file :".$_FILES["image_file"]["error"];
  die();
  }
...
if(isset($_SESSION['isAdmin'])){
	$finalFileName = "emails.xlsx";//$_FILES["image_file"]["name"];
	echo $finalFileName;
}
else{
...
if (file_exists("../files/" . $finalFileName))
      {
      echo $finalFileName . " already exists.\n ";
      unlink("../files/" . $finalFileName);
      echo "file Deleted...";
        move_uploaded_file($_FILES["image_file"]["tmp_name"],
      "../files/" . $finalFileName);
echo <<<EOF
